...
function MyListings() {
  ...
  useEffect(() => {
    ...
    const fetchBooks = async () => {
      try {
        ...
        setBooks(response.data.books || []); // Ensure empty array if no data
      } catch (err) {
  ...
  const handleDelete = async (bookId) => {
    ...
    try {
      await axios.delete(`https://localhost:5000/api/books/${bookId}`, {
        headers: { Authorization: `Bearer ${userToken}` },
      });
  ...
  return (
    <div className="min-h-screen bg-grey p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {books.map((book) => (
          <div key={book._id} className="bg-white border border-grey p-4 rounded-lg shadow-md">
            {book.imageUrl ? (
              <img src={book.imageUrl} alt={book.bookName} className="h-40 w-full object-cover mb-4 rounded" />
            ) : (
              <div className="h-40 bg-grey flex items-center justify-center mb-4">
                <p className="text-black">No Image</p>
              </div>
            )}
            <h2 className="text-lg font-semibold">{book.bookName}</h2>
            <p className="text-sm text-grey">College: {book.collegeName}</p>
            <p className="text-sm text-black">Price: ₹{book.price}</p>
            <button onClick={() => handleDelete(book._id)} className="mt-4 px-4 py-2 bg-red text-white rounded">
              Delete
            </button>
          </div>
        ))}
